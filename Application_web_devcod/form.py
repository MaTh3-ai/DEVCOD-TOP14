...
def inscription():
    if request.method == 'POST':
        ...
        if user_db.insert_user(username, password):
            return redirect(url_for('form.acceuil'))  # Rediriger vers la page d'accueil après l'inscription
        else:
...
@form.route('/accueil')
def acceuil():
    return render_template('accueil.html')  # Page d'accueil

if __name__ == "__main__":
    webbrowser.open('http://127.0.0.1:5000')  # Ouvrir le navigateur à l'adresse spécifiée